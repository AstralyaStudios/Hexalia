...
final class HerbSpreading {
    ...
    static void spread(ServerLevel level, RandomSource random, BlockPos origin, BlockState state, boolean aquatic) {
        int remaining = 8;
    ...
    private static boolean isValidTarget(ServerLevel level, BlockPos pos, BlockState state, boolean aquatic) {
        boolean available = aquatic
                ? level.getBlockState(pos).is(Blocks.WATER) && level.getFluidState(pos).is(FluidTags.WATER)
                : level.isEmptyBlock(pos);
        return available && state.canSurvive(level, pos);
    }
    private static BlockPos step(BlockPos origin, RandomSource random) {
        return origin.offset(random.nextInt(3) - 1, random.nextInt(2) - random.nextInt(2), random.nextInt(3) - 1);
    }
}
