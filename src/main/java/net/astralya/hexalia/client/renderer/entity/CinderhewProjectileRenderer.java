...
public final class CinderhewProjectileRenderer extends EntityRenderer<CinderhewProjectile> {
    ...
    @Override
    public void render(CinderhewProjectile projectile, float entityYaw, float partialTick,
        ...
        if (!projectile.isEmbedded()) poseStack.mulPose(Axis.XP.rotationDegrees((projectile.tickCount + partialTick) * DEGREES_PER_TICK));
        itemRenderer.renderStatic(projectile.getCarriedStack(), ItemDisplayContext.FIXED, packedLight, OverlayTexture.NO_OVERLAY,
                poseStack, buffer, projectile.level(), projectile.getId());
        poseStack.popPose();
        super.render(projectile, entityYaw, partialTick, poseStack, buffer, packedLight);
    }

    @Override
    public ResourceLocation getTextureLocation(CinderhewProjectile projectile) { return InventoryMenu.BLOCK_ATLAS; }
}
