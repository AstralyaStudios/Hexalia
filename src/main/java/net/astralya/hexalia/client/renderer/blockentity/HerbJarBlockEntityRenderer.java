...
public final class HerbJarBlockEntityRenderer implements BlockEntityRenderer<HerbJarBlockEntity> {
    ...
    @Override
    public void render(HerbJarBlockEntity herbJar, float partialTick, PoseStack poseStack,
        ...
        renderContents(herbJar, storedItem, visibleLayers, level, poseStack, buffer, packedLight, packedOverlay);
    ...
    private void renderContents(HerbJarBlockEntity herbJar, ItemStack storedItem, int visibleLayers,
                                Level level, PoseStack poseStack, MultiBufferSource buffer,
        ...
        for (int layer = 0; layer < visibleLayers; layer++) {
            ...
            itemRenderer.renderStatic(storedItem, ItemDisplayContext.GROUND, packedLight, packedOverlay,
                    poseStack, buffer, level, (int) (herbJar.getBlockPos().asLong() + layer));
            poseStack.popPose();
        }
    }

    private void renderLabel(HerbJarBlockEntity herbJar, ItemStack storedItem, Level level,
        ...
        itemRenderer.renderStatic(storedItem, ItemDisplayContext.FIXED, packedLight, packedOverlay,
                poseStack, buffer, level, (int) (herbJar.getBlockPos().asLong() ^ 31L));
        poseStack.popPose();
    }
    ...
    private static int occupiedSlots(HerbJarBlockEntity herbJar) {
        int occupied = 0;
        for (int slot = 0; slot < herbJar.getContainerSize(); slot++) {
            if (!herbJar.getItem(slot).isEmpty()) occupied++;
        }
        return occupied;
    }
}
