...
public final class CinderhewProjectile extends AbstractArrow {
    private static final EntityDataAccessor<ItemStack> CARRIED_STACK = SynchedEntityData.defineId(CinderhewProjectile.class, EntityDataSerializers.ITEM_STACK);
    private static final EntityDataAccessor<Boolean> RETURNING = SynchedEntityData.defineId(CinderhewProjectile.class, EntityDataSerializers.BOOLEAN);
    ...
    public CinderhewProjectile(EntityType<? extends CinderhewProjectile> type, Level level, LivingEntity owner, ItemStack carriedStack) {
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(CARRIED_STACK, ItemStack.EMPTY);
        entityData.define(RETURNING, false);
    }

    public ItemStack getCarriedStack() { return entityData.get(CARRIED_STACK); }
    public boolean isReturning() { return entityData.get(RETURNING); }
    public boolean isEmbedded() { return inGround && !isReturning(); }
    private void setCarriedStack(ItemStack stack) { entityData.set(CARRIED_STACK, stack.copy()); }
    ...
    protected void onHitEntity(EntityHitResult result) {
        if (level().isClientSide || dealtDamage || isReturning()) return;
        Entity target = result.getEntity();
        Entity owner = getOwner();
        if (!target.hurt(damageSources().arrow(this, owner), 7.0F)) return;
        dealtDamage = true;
        if (target instanceof LivingEntity livingTarget) {
            ...
            if (!livingTarget.fireImmune()) livingTarget.setSecondsOnFire(2);
        }
        ItemStack stack = getCarriedStack().copy();
        if (owner instanceof LivingEntity livingOwner) stack.hurtAndBreak(1, livingOwner, entity -> { });
    ...
    protected void onHitBlock(BlockHitResult result) {
        ...
        if (!level().isClientSide) beginReturning();
    }
    ...
    private void beginReturning() {
        ...
        setNoPhysics(true);
        inGround = false;
        level().playSound(null, blockPosition(), SoundEvents.TRIDENT_RETURN, SoundSource.PLAYERS, 0.55F, 1.15F);
    }

    private void tickReturn() {
        Entity owner = getOwner();
        if (!(owner instanceof Player player) || !player.isAlive() || player.isRemoved() || player.level() != level()) {
            dropCarriedStack();
            return;
        }
        ...
        if (velocity.lengthSqr() > MAX_RETURN_SPEED * MAX_RETURN_SPEED) velocity = velocity.normalize().scale(MAX_RETURN_SPEED);
    ...
    private void recover(Player player) {
        ...
        level().playSound(null, player.blockPosition(), SoundEvents.ITEM_PICKUP, SoundSource.PLAYERS, 0.45F, 1.1F);
        discard();
    }
    ...
    @Override
    protected ItemStack getPickupItem() { return getCarriedStack().copy(); }

    @Override
    public void addAdditionalSaveData(CompoundTag tag) {
        super.addAdditionalSaveData(tag);
        ItemStack stack = getCarriedStack();
        if (!stack.isEmpty()) tag.put(TAG_STACK, stack.save(new CompoundTag()));
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        if (tag.contains(TAG_STACK)) setCarriedStack(ItemStack.of(tag.getCompound(TAG_STACK)));
